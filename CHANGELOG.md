...
0.4: voice end to end: hold T push to talk in every dialog (Cloud STT) and spoken replies per keeper kind (Cloud TTS); dream runs carry a run_id from POST /dream so the graph movie plays when consolidation lands; internal routes require the deploy token; deploy defaults to free-tier scale-to-zero. 52 backend + 721 frontend tests.

...
- **Runs are bounded.** `runtime.run_agent` caps one run at `MAX_LLM_CALLS` (12) model calls; a model that keeps calling tools stops there and the flow's deterministic fallback answers.

...
- **Dream prose agents.** Single completions used by the dreaming box: `dream_select(themes, cap)` (sorts the linker's candidates into human patterns and technical terms; only the human bucket is born: fears, wishes, obsessions, ambitions, people, places, a work read as a pattern of the mind; technology, tool, project and event names fall away; returns kept keys, every candidate when the model fails), `dream_write` (a dark keeper's name, persona and book from a theme) and `dream_narrative` (the night's summary).
...
| `keeper_tell(world, kid, text)` async | user message | `{reply, book?, book_grown?}`; a follow-up naming an existing book grows it (`extends_slug` from the model, validated against her shelf, appended as a dated section) and returns `book_grown` instead of `book`; a passing remark (`note: true` from the model, or a telling under `SHORT_REMARK` chars whose authored body stays under the small tier) lands as a dated entry in her one notes book (slug `notes`, `about_slugs` validated against her shelf and kept as its links) and returns it as `book_grown`; a model that does not answer leaves a reply saying so and the raw text as the body (a short single sentence goes to the notes book instead); the user turn recorded in her session names the slug the tell shelved; the keeper authors the book's full body from the told memory (markdown, sized by the material; the raw text is the fallback body); dark keepers reply in archetype voice, never write; `LIBRARY_FULL` raises after the turns are still recorded |
...
| `dark_chat(world, kid, text)` async | user message | `{reply, book: null, book_grown: null, sources}` (`sources`: index rows with `keeper_id` of the village books she opened); the ridge conversation above; `keeper_say` sends every dark keeper here with `kind: "talk"` |

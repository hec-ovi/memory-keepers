...
- **Runs are bounded.** `runtime.run_agent` caps one run at `MAX_LLM_CALLS` (12) model calls. `runtime.run_flow` runs a flow with its tools and, when a model spends that budget on tools, runs it once more without them, so the answer is still the model's own; only a second failure reaches the deterministic fallback.

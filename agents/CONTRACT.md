...
- **Dream prose agents.** Single completions used by the dreaming box: `dream_select(themes, cap)` (which of the linker's candidates are about the person: fears, wishes, obsessions, ambitions, people, places; technology, tool and product names are dropped; returns kept keys, every candidate when the model fails), `dream_write` (a dark keeper's name, persona and book from a theme) and `dream_narrative` (the night's summary).

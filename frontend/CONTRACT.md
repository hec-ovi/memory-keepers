...
| `keeper:selected` | `{ keeperId }` (consumers also accept a bare id) | render/scene_overworld (pick: the keeper herself OR any part of her cottage; the door alone keeps its enter pick), render/scene_interior (clicking the keeper herself, the "Sit beside her" chairs view), ui/keepers_list (row click) | main.js (records `state.selectedKeeperId`, so a scene that builds after the emit still sees the selection), ui/dialog (open), render/scene_overworld (select-to-follow: camera tweens to the standard framing and follows her; any user camera input or a deselect exits) |

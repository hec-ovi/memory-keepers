...
| `book:created` | `{ keeperId, book }` | ui/dialog (tell reply that carries a book; unconscious tells write none) | main.js, ui/hud, render/scene_interior, render/audio |
| `book:destroyed` | `{ keeperId, slug }` | ui/reader (Destroy) | main.js, ui/hud, render/scene_interior |
| `book:open` | `{ keeperId, slug }` | ui/dialog (consulted book icon), render/scene_interior (fetched book) | ui/reader (open), render/audio |
...
| `reader:closed` | none | ui/reader (every close) | render/scene_interior, render/audio |
...
| `graph:wave` | `{ index, t }` | render/scene_graph | render/audio |

...
// One height field shapes everything: terrain, buildings and walkers all
// sample heightAt, so nothing ever sinks into a hill.
...
const GRASS_DARK = new THREE.Color(0x1c1533);
const SAND = new THREE.Color(0xd9c489);
const DIRT = new THREE.Color(0xb99c6b);
const BASE_Y = 0.12;

export function makeHeightField(layout) {
  const rng = mulberry32(hashSeed("terrain"));
  const bumps = [];
  for (let i = 0; i < 10; i++) {
    bumps.push({ x: (rng() - 0.5) * 70, z: (rng() - 0.5) * 70,
                 r: 8 + rng() * 14, h: 0.8 + rng() * 2.2 });
  }
  // Ground stays flat near everything functional: hubs, doors, junctions,
  // and the middle of every path segment.
  const flat = [PLAZA, DARK_HEART];
  for (const node of Object.values(layout.network.nodes)) flat.push(node);
  for (const [a, b] of layout.network.edges) {
    const pa = layout.network.nodes[a], pb = layout.network.nodes[b];
    flat.push({ x: (pa.x + pb.x) / 2, z: (pa.z + pb.z) / 2 });
  }
  for (const plot of layout.plots) flat.push(plot);

  return function heightAt(x, z) {
    const shore = islandRadiusAt(Math.atan2(z, x));
    const edge = Math.hypot(x, z) / shore;
    if (edge >= 1.02) return -1.6;
    let bump = 0;
    for (const b of bumps) {
      const d = Math.hypot(x - b.x, z - b.z);
      if (d < b.r) bump += Math.cos((d / b.r) * Math.PI * 0.5) * b.h;
    }
    let clearing = 1;
    for (const p of flat) {
      const d = Math.hypot(x - p.x, z - p.z);
      if (d < 4) return shoreDrop(BASE_Y, edge);
      if (d < 11) clearing = Math.min(clearing, (d - 4) / 7);
    }
    return shoreDrop(BASE_Y + bump * clearing * clearing, edge);
  };
}

function shoreDrop(y, edge) {
  if (edge <= 0.9) return y;
  const t = Math.min(1, (edge - 0.9) / 0.11);
  return y * (1 - t) - 1.6 * t * t;
}

export function buildIsland(scene, layout) {
  const group = new THREE.Group();
  const heightAt = makeHeightField(layout);

  // Terrain mesh sampled from the height field, vertex-color blends.
  const geo = new THREE.CircleGeometry(1, 200, 0, Math.PI * 2);
  geo.rotateX(-Math.PI / 2);
  const pos = geo.attributes.position;
  const colors = [];
  for (let i = 0; i < pos.count; i++) {
    const x0 = pos.getX(i), z0 = pos.getZ(i);
    const shore = islandRadiusAt(Math.atan2(z0, x0));
    const x = x0 * shore, z = z0 * shore;
    const y = heightAt(x, z);
    pos.setXYZ(i, x, y, z);

    // the black island: the north side reads as night ground on its own
    const darkness = THREE.MathUtils.clamp((4 - z) / 14, 0, 1);
    const color = GRASS.clone().lerp(GRASS_DARK, darkness);
    const edge = Math.hypot(x0, z0);
    color.lerp(SAND, THREE.MathUtils.smoothstep(edge, 0.86, 0.97));
    if (y > BASE_Y + 1.6) color.lerp(new THREE.Color(0x8a8a7a), 0.35); // rocky tops
  ...
  // Paths: flat ribbons along network edges, resting on the (flattened) ground.
  const pathMat = new THREE.MeshStandardMaterial({ color: DIRT, roughness: 1 });
  const darkPathMat = new THREE.MeshStandardMaterial({ color: 0x453a6b, roughness: 1 });
  const { nodes, edges } = layout.network;
  for (const [a, b] of edges) {
    const pa = nodes[a], pb = nodes[b];
    const length = Math.hypot(pb.x - pa.x, pb.z - pa.z);
    const dark = (pa.z + pb.z) / 2 < -6;
    const ribbon = new THREE.Mesh(new THREE.PlaneGeometry(length, 1.5),
      dark ? darkPathMat : pathMat);
    ribbon.rotateX(-Math.PI / 2);
    ribbon.position.set((pa.x + pb.x) / 2, BASE_Y + 0.03, (pa.z + pb.z) / 2);
  ...
  for (const [center, radius, color] of [[PLAZA, 6.5, DIRT], [DARK_HEART, 5, 0x2c2350]]) {
    ...
    disc.position.set(center.x, BASE_Y + 0.04, center.z);
    group.add(disc);
  }
  ...
  monument.position.set(PLAZA.x, BASE_Y + 0.35, PLAZA.z);
  ...
  spire.position.set(DARK_HEART.x, BASE_Y + 3.5, DARK_HEART.z);
  group.add(spire);
  const spireGlow = new THREE.PointLight(0x6a54d8, 1.6, 22);
  spireGlow.position.set(DARK_HEART.x, 5, DARK_HEART.z);
  group.add(spireGlow);
  ...
  scene.add(group);
  return { group, waterUpdate, terrain, heightAt };
}
